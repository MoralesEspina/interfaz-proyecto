...
import { Pasajero, PasajeroService } from '../services/pasajero.service';
...
export class MostrarPasajerosTablaComponent implements OnInit {
  ...
  constructor(private VueloService:VueloService, private router:Router, private fb: FormBuilder,
    private pasajeroService: PasajeroService, private http:HttpClient, private personaService:PersonaService) { }
  ...
  obtenerID(){
    ...
    this.personaService.getId(this.persona).subscribe(
    res => {
      this.persona = res[0];
      console.log(res[0]);
    },
